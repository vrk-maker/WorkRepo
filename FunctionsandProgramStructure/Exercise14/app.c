...
void display() {
    ...
    for (int i = top; i >= 0; i--) {
        printf("Variable name: %c\tValue: %d\n", stack[i].name, stack[i].value);
    }
}

/**
 * @brief Main function to run the variable stack program.
 * @return Status code
 */
int main() {
    char command; ///< Command entered by the user
    char var_name; ///< Name of the variable
    int var_value; ///< Value of the variable
    ...
    while (1) {
        ...
        struct stackvalues popvalue; // Declare popvalue here
        ...
        switch (command) {
            case 'a':
                ...
                printf("Variable '%c' assigned with value %d\n", var_name, var_value);
                break;
            case 'r':
            ...
            case 'p':
                ...
                printf("Popped variable: %c\tValue: %d\n", popvalue.name, popvalue.value);
                break;
            default:
